...
def margin(company_name, strike_price, ce_or_pe, expiry_date):
    ...
    url = 'https://dev-in-2.upstox.com/calculators/span/span.php?action=calculateMargin&signature=7ef16bb143db4aaf4465c4046809f495c063f64f&nonce=1956202059&timestamp=1604640863&myProducts=%5B%5B%22NSE%22%2C%22Options%22%2C%22' + \
        str(company_name) + '%22%2C%22'+str(expiry_date)+'%22%2C%22'+str(ce_or_pe) + \
        '%22%2C%22'+str(strike_price)+'%22%2C-' + str(lot) + '%2C%22%22%5D%5D'
    r = requests.get(url)
    data = r.json()
...
def get_option_data(company):
    ...
    df.drop(['CE.strikePrice', 'CE.expiryDate', 'CE.identifier', 'CE.openInterest', 'CE.changeinOpenInterest', 'CE.pchangeinOpenInterest', 'CE.totalTradedVolume', 'CE.impliedVolatility', 'CE.change', 'CE.pChange', 'CE.totalSellQuantity', 'CE.askQty', 'CE.totalBuyQuantity', 'CE.askPrice', 'CE.underlyingValue', 'PE.strikePrice',
             'PE.expiryDate', 'PE.underlying', 'PE.identifier', 'PE.openInterest', 'PE.changeinOpenInterest', 'PE.pchangeinOpenInterest', 'PE.totalTradedVolume', 'PE.impliedVolatility', 'PE.change', 'PE.pChange', 'PE.totalBuyQuantity', 'PE.totalSellQuantity', 'PE.askQty', 'PE.askPrice', 'PE.underlyingValue'], axis=1, inplace=True)
    return df
...
d = {'company': [], 'expiry date': [], 'strike price': [], 'stock price': [], 'margin': [], 'iscall': [], 'lot': [], 'call ltp': [],
     'call bid price': [], 'call bid quantity': [], 'put ltp': [], 'put bid price': [], 'put bid quantity': [], 'percentage change in price': []}
...
def fetch_main(list_of_company):
    ...
    for company in list_of_company:
        try:
            ...
            df, ltp, crontime = oi_chain_builder(str(company), expiry=str(
                expiry_var_date)+"-" + str(expiry_var_lower) + "-2021")
            # df = get_option_data(str(company))
            df = df.drop(columns=['CALLS_Chart', 'CALLS_OI', 'CALLS_Chng in OI', 'CALLS_Volume',
                                  'CALLS_IV', 'CALLS_Net Chng',
                                  'CALLS_Ask Price', 'CALLS_Ask Qty',
                                  'PUTS_Ask Price', 'PUTS_Ask Qty',
                                  'PUTS_Net Chng', 'PUTS_IV', 'PUTS_Volume',
                                  'PUTS_Chng in OI', 'PUTS_OI', 'PUTS_Chart'])
        ...
        df['percentage change in price'] = ((df['Strike Price']-ltp)/ltp)*100
        df = df[((df['percentage change in price'] > 5) | ((df['percentage change in price'] < -5)))]
        df['margin'] = ''
        strike_price_array = df['Strike Price']
        logger.info("Margin statred for " + company)
        df = df.reset_index(drop=True)
        for i, strike in enumerate(strike_price_array):
            ce_or_pe = "C" if (
                df['percentage change in price'][i] > 5) else "P"
            data, lot = margin(company, strike, ce_or_pe, df['expiryDate'][i])
            print("Got margin for "+str(strike))
            df['margin'][i] = data
            df2 = pd.DataFrame([[company, df['expiryDate'][i], strike, ltp, data, ce_or_pe, lot, df['CALLS_LTP'][i], df['CALLS_Bid Price'][i], df['CALLS_Bid Qty'][i], df['PUTS_LTP'][i], df['PUTS_Bid Price'][i], df['PUTS_Bid Qty'][i], df['percentage change in price'][i]]], columns=[
                               'company', 'expiry date', 'strike price', 'stock price', 'margin', 'iscall', 'lot', 'call ltp', 'call bid price', 'call bid quantity', 'put ltp', 'put bid price', 'put bid quantity', 'percentage change in price'])
            const = final_dataframe.append(df2, ignore_index=True)
            final_dataframe = const
        else:
            print("remaning company : " +
                  str(len(list_of_company) - company_number_count))
        company_number_count += 1

expiry_date = str(expiry_var)+"+"+ str(expiry_var_date)+"+"+str(currentYear)
fetch_main(list_of_company)
date_after_month = datetime.today() + relativedelta(months=1)
nextMonth = date_after_month.strftime('%m')
nextYear = date_after_month.strftime('%Y')
nextDay = date_after_month.strftime('%d')
nextMonthText = date_after_month.strftime('%h')
nextDay = int(nextDay) 
nextMonth = int(nextMonth)
nextYear = int(nextYear)
date_th=(datetime(nextYear,nextMonth,nextDay) + relativedelta(day=31, weekday=TH(-1))).strftime("%d")
expiry_var_date = date_th
expiry_var = nextMonthText
expiry_var=expiry_var.upper() 
expiry_var_lower = expiry_var.title()
print(expiry_var_date,expiry_var)
expiry_date = str(expiry_var)+"+"+ str(expiry_var_date)+"+"+str(currentYear)
fetch_main(list_of_company)
...
def adding_percentage_margin():
    ...
    for i, strike in enumerate(final_dataframe['strike price']):
        ...
        try:
            final_dataframe['% margin'][i] = float(
                ltp) * float(final_dataframe['lot'][i]) * 100 / (final_dataframe['margin'][i])
        except Exception as error:
            print(error)
...
# %%
final_dataframe.to_csv(str(currentDay) + "-" +
                       str(currentMonth) + '_upstox.csv', index=False)
